...
public class ArticleDAO extends AbstractDAO<ArticleEntity, UUID> implements ArticleRepository {
  ...
  public PageResult<Article> findMostRecentArticlesByFilter(ArticleFilter articleFilter) {
    ...
    query.setParameter("loggedUserId", articleFilter.loggedUserId());

    // Handle paging
    int offset = articleFilter.offset();
    int limit = articleFilter.limit();
    ...
    long total = countTotalArticles(articleFilter.loggedUserId());
  ...
  public PageResult<Article> findArticlesByFilter(ArticleFilter filter) {
    ...
    configFilterFindArticlesQueryBuilder(
        findArticlesQueryBuilder, filter.tags(), filter.authors(), filter.favorited(), params);

    configFilterFindArticlesQueryBuilder(
        countArticlesQueryBuilder, filter.tags(), filter.authors(), filter.favorited(), params);
  ...
  private void configFilterFindArticlesQueryBuilder(
    ...
    findArticlesQueryBuilder.updateQueryStatementConditional(
        isNotEmpty(tags),
        "inner join a.tags t inner join t.tag as tag",
        "upper(tag.name) in :tags",
        () -> params.put("tags", finalTagsUpper));

    findArticlesQueryBuilder.updateQueryStatementConditional(
        isNotEmpty(authors),
        "inner join a.author as authors",
        "upper(authors.username) in :authors",
        () -> params.put("authors", finalAuthorsUpper));
    ...
    findArticlesQueryBuilder.updateQueryStatementConditional(
        ...
        () -> params.put("favorites", finalFavsUpper));
  }
}
